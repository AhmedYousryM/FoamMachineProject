...
void mainArduino::mainsendInfo(Message msg){
...
String mainArduino::mainReading(HardwareSerial& comserial,char* readarr){
...
void mainArduino::main_set_serial_for_queue(){
  /*
  Serial 2 ->> CNC
  Serial 1 ->> ResPi
  Serial 3 ->> Fork
...
Message mainArduino::mainSelectedMessagefromQueues(){
...
void mainArduino::main_stop_waiting(uint8_t select){
...
void mainArduino::main_send(Message msg,uint8_t select){
    uint8_t lengt;
...
void mainArduino::mainSendMessage(){
...
void mainArduino::SendingWhat(Message msg,InformationQueue Queue){
  String Msg;
...
InformationQueue* mainArduino::SelectQueuefromSerial(HardwareSerial& comserial){
...
InformationQueue* mainArduino::SelectQueueforOk(uint8_t select){
  switch (select){
    case 1:
...
void mainArduino::mainRecieveMessage(HardwareSerial& comserial){
